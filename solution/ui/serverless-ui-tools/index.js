...
class ServerlessUIToolsPlugin {
  constructor(serverless, options) {
    ...
    this.commands = {
      "publish-ui": {
        usage:
          'Packages UI & Deploys (via "aws s3 sync") a target directory to the `websiteBucketName` bucket.',
    ...
    this.cli = {
      ...
      log(message) {
        serverless.cli.consoleLog(
          `[serverless-ui-tools] ${chalk.yellowBright(message)}`
        );
      }
    };
  }
  ...
  async deploy() {
    const bucketName = this.serverless.service.custom.settings
      .websiteBucketName;
  ...
  writeEnv() {
    ...
    this.cli.log(
      `Writing Create React App environment file "${fileName}" with the following content:\n`
    );
  ...
  async invalidateCache() {
    const distributionId = this.serverless.service.custom.settings
      .websiteCloudFrontId;
    const shouldInvalidate = this.options["invalidate-cache"];
    if (shouldInvalidate && !distributionId) {
      throw Error(
        'You specified "--invalidate-cache", but `websiteCloudFrontId` setting was not found'
      );
    }
    ...
    if (shouldInvalidate && this.mutatedBucket) {
      ...
      try {
        ...
        throw new Error(
          `Error invalidating CloudFront ${distributionId} cache: ${err}`
        );
      }
    }
    ...
    if (this.mutatedBucket) {
      ...
      this.cli.log(
        'Nothing new to deploy. Did you forget to call "package-ui"? Skipping deployment...'
      );
    }
  }
